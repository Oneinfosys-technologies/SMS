...
class CalculatePayHeads
{
    ...
    public function execute(SalaryTemplate $salaryTemplate, array $records, array $params = [])
    {
        ...
        $grossEarning = collect($payHeads)->where('category', 'earning')->sum('amount');
        $grossEarning = SysHelper::formatAmount($grossEarning);
        $earningComponent = collect($payHeads)->where('category', 'earning')->sum('amount');
        $earningComponent = SysHelper::formatAmount($earningComponent);
        ...
        $grossDeduction = collect($payHeads)->where('category', 'deduction')->sum('amount');
        $grossDeduction = SysHelper::formatAmount($grossDeduction);
        $deductionComponent = collect($payHeads)->where('category', 'deduction')->sum('amount');
        $deductionComponent = SysHelper::formatAmount($deductionComponent);
        ...
        $employeeContribution = SysHelper::formatAmount($employeeContribution);
        ...
        $employerContribution = SysHelper::formatAmount($employerContribution);
    ...
    private function getPayHeads(SalaryTemplateRecord $record, array $payHeads = [], array $records = [], array $params = [])
    {
        ...
        if ($record->type == PayHeadType::FLAT_RATE) {
            ...
            $payHeads[] = [
                ...
                'category' => $record->payHead->category->value,
                'code' => $record->payHead->code,
                'amount' => SysHelper::formatAmount(($inputRecordAmount / $params['monthly_days']) * $params['working_days']),
            ];
            ...
            $amount = SysHelper::formatAmount($value * $inputRecordAmount);
    ...
    private function evaluateNonConditionalFormula(SalaryTemplateRecord $record, array $payHeads, array $params = [])
    {
        ...
        $evaluation = SysHelper::formatAmount($evaluation);
