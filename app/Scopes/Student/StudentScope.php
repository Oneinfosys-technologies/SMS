...
trait StudentScope
{
    ...
    {
        $query
            ->select('students.id', 'students.uuid', 'students.batch_id', 'students.period_id', 'students.contact_id', 'students.fee_structure_id', 'students.start_date', 'students.end_date', 'students.cancelled_at', 'contacts.team_id', 'contacts.user_id')
    ...
    {
        $query
            ->select('students.*', \DB::raw('REGEXP_REPLACE(CONCAT_WS(" ", contacts.first_name, contacts.middle_name, contacts.third_name, contacts.last_name), "[[:space:]]+", " ") as name'), 'admissions.code_number', 'admissions.joining_date', 'admissions.leaving_date', 'batches.uuid as batch_uuid', 'batches.name as batch_name', 'courses.uuid as course_uuid', 'courses.name as course_name', 'courses.term as course_term', 'contacts.team_id', 'contacts.user_id', 'contacts.gender', 'contacts.photo', 'contacts.birth_date', 'contacts.father_name', 'contacts.mother_name', 'contacts.contact_number', 'enrollment_types.name as enrollment_type_name')
            ->join('contacts', 'students.contact_id', '=', 'contacts.id')
            ->join('batches', 'students.batch_id', '=', 'batches.id')
            ->join('courses', 'batches.course_id', '=', 'courses.id')
            ->join('admissions', 'students.admission_id', '=', 'admissions.id')
